...
const CONFIG = {
  ...
  // Discord -> Twister MTA
  twisterChannelId: String(process.env.TWISTER_CHANNEL_ID || '1507898085370499263').trim(),
...
function bridgePingHandler(req, res) {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V20_DISCORD_TO_MTA_FIXED', now: Date.now() });
}
...
function selectNextTwisterJob() {
  ...
  // V20: si MTA tomó un mensaje pero no devolvió result por error/restart,
  // se libera otra vez. Antes podía quedarse processing y desaparecer de Discord -> MTA.
  for (const job of twisterJobs.values()) {
    if (job.status === 'processing' && job.lockedAt && now - job.lockedAt > 10000) {
      job.status = 'pending';
      job.lockedAt = 0;
      console.log(`[TWISTER REQUEUE] id=${job.id} volvió a pending por timeout de MTA`);
    }
  }
  ...
  for (const job of twisterJobs.values()) {
    if (job.status === 'pending') {
      job.status = 'processing';
      job.lockedAt = now;
      console.log(`[TWISTER->MTA NEXT] id=${job.id} user=${job.discordId} text=${String(job.text || '').slice(0, 80)}`);
      return job;
    }
  }
  return null;
...
function bridgeTwisterNextHandler(req, res) {
  ...
  if (!job) {
    return res.json({
      ok: true,
      job: null,
      pending: [...twisterJobs.values()].filter(j => j.status === 'pending').length,
      processing: [...twisterJobs.values()].filter(j => j.status === 'processing').length,
    });
  }
  return res.json({
    ok: true,
...
async function bridgeTwisterFromMtaHandler(req, res) {
  if (!okAuth(req, res)) return;
  lastBridgeSeenAt = Date.now();
  const body = mergedInput(req);

  const playerName = cleanTwisterText(body.playerName || 'Jugador').slice(0, 32);
  const text = cleanTwisterText(body.text || '');
  const accountName = cleanTwisterText(body.accountName || '').slice(0, 48);
  ...
  try {
    const channel = await client.channels.fetch(CONFIG.twisterChannelId);
    if (!channel || !channel.isTextBased()) {
      console.log('[MTA->DISCORD TWISTER] canal inválido o no es texto');
      return;
    }

    const suffix = accountName ? `\n> Cuenta: **${accountName}**` : '';
    await channel.send({
      content: `🐦 **Twister Santiago RP**\n👤 **${playerName}:** ${text}${suffix}`,
      allowedMentions: { parse: [] },
    });
    console.log(`[MTA->DISCORD TWISTER] ${playerName}: ${text.slice(0, 80)}`);
  } catch (e) {
    console.error('[MTA->DISCORD TWISTER ERROR]', e);
  }
}
...
function bridgeTwisterResultHandler(req, res) {
  ...
  if (id && twisterJobs.has(id)) {
    if (okValue) {
      twisterJobs.delete(id);
      console.log(`[TWISTER RESULT] id=${id} publicado en MTA y eliminado de cola`);
    } else {
      const job = twisterJobs.get(id);
      job.status = 'pending';
      job.lockedAt = 0;
      console.log(`[TWISTER RESULT] id=${id} NO publicado, vuelve a pending error=${body.error || ''}`);
    }
  }
  res.json({ ok: true, received: true });
}
...
app.get('/', (_req, res) => {
  res.status(200).send('Santiago RP Verify Bot FAST V20 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V20_DISCORD_TO_MTA_FIXED',
...
app.get('/bridge/twister/from-mta', (req, res) => bridgeTwisterFromMtaHandler(req, res).catch(e => { console.error('[TWISTER FROM MTA ROUTE ERROR]', e); res.status(500).json({ ok: false, error: 'route_error' }); }));
app.post('/bridge/twister/from-mta', (req, res) => bridgeTwisterFromMtaHandler(req, res).catch(e => { console.error('[TWISTER FROM MTA ROUTE ERROR]', e); res.status(500).json({ ok: false, error: 'route_error' }); }));
...
setInterval(() => {
  console.log(`[STATUS] verifyPending=${countJobs('pending')} verifyProcessing=${countJobs('processing')} twisterPending=${[...twisterJobs.values()].filter(j => j.status === 'pending').length} twisterProcessing=${[...twisterJobs.values()].filter(j => j.status === 'processing').length} bridgeAgo=${msAgo(lastBridgeSeenAt)} timeout=${CONFIG.jobTimeoutMs}ms`);
}, 60000);
setInterval(() => {
  const now = Date.now();
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V20 activo en puerto ${CONFIG.port}`);
});
