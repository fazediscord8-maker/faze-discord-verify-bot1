...
function bridgePingHandler(req, res) {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V18_TWISTER_2WAY', now: Date.now() });
}
...
app.get('/', (_req, res) => {
  res.status(200).send('Santiago RP Verify Bot FAST V18 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V18_TWISTER_2WAY',
