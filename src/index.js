...
const CONFIG = {
  ...
  // V14: timeout seguro configurable. Default 120s; máximo 180s.
...
async function sendVerifyWebhookLog(kind, payload) {
  ...
  const fields = isSuccess ? [
    ...
    { name: '👤 Usuario Discord', value: payload.discordLabel || 'No disponible', inline: false },
    { name: '🆔 Discord ID', value: String(payload.discordId || 'No disponible'), inline: true },
    { name: '🎮 Nickname introducido', value: String(payload.nickname || 'No disponible').slice(0, 64), inline: true },
...
function bridgePingHandler(req, res) {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V14_STABLE_NO_DOUBLE_CLICK', now: Date.now() });
}
...
app.get('/', (_req, res) => {
  res.status(200).send('Santiago RP Verify Bot FAST V14 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V14_STABLE_NO_DOUBLE_CLICK',
...
async function handleInteraction(interaction) {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'faze_verify_modal') {
    ...
    // V14: no mensaje intermedio. Solo resultado final o corte seguro configurable.
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V14 activo en puerto ${CONFIG.port}`);
});
