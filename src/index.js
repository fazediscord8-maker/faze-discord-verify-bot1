...
const CONFIG = {
  ...
  jobTimeoutMs: Number(process.env.JOB_TIMEOUT_MS || 90000),
...
app.get('/', (_req, res) => {
  res.status(200).send('FAZE Discord Verify Bot FAST V7 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V7_SAFE_BRIDGE',
...
app.post('/bridge/ping', (req, res) => {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V7_SAFE_BRIDGE', now: Date.now() });
});
...
async function sendPanel() {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x2ecc71)
    .setFooter({ text: 'Sistema de Verificación Faze • FAST V7' });
...
async function handleInteraction(interaction) {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'faze_verify_modal') {
    ...
    await safeEditReply(interaction, '⏳ Verificando tu cuenta con el servidor MTA... espera unos segundos.');

    setTimeout(async () => {
      const job = jobs.get(id);
      if (job && (job.status === 'pending' || job.status === 'processing')) {
        jobs.delete(id);
        console.log(`[TIMEOUT] id=${id} waited=${Date.now() - job.createdAt}ms attempts=${job.attempts} bridgeAgo=${msAgo(lastBridgeSeenAt)}`);
        ...
        await safeEditReply(interaction, `❌ La verificación tardó demasiado. Inténtalo otra vez en unos segundos.${bridgeMsg}`);
      }
    }, CONFIG.jobTimeoutMs);
  }
}
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V7 activo en puerto ${CONFIG.port}`);
});
