...
app.get('/', (_req, res) => {
  res.status(200).send('FAZE Discord Verify Bot FAST V4 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V4_SAFE_INTERACTIONS',
...
app.post('/bridge/ping', (req, res) => {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V4_SAFE_INTERACTIONS', now: Date.now() });
});
...
async function sendPanel() {
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(0x2ecc71)
    .setFooter({ text: 'Sistema de Verificación Faze • FAST V4' });
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V4 activo en puerto ${CONFIG.port}`);
});
