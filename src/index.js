...
async function bridgeTwisterFromMtaHandler(req, res) {
  ...
  const body = mergedInput(req);
  const playerName = cleanTwisterText(body.playerName || body.name || 'Jugador').slice(0, 32) || 'Jugador';
  const text = cleanTwisterText(body.text || body.message || '');
  const tag = cleanTwisterText(body.tag || 'Twister').slice(0, 64);
  ...
  try {
    ...
    if (!channel || !channel.isTextBased()) {
      console.log('[MTA->DISCORD TWISTER] Canal inválido o no es de texto:', CONFIG.twisterChannelId);
      return;
    }

    const extra = accountName ? `\n> Cuenta: **${accountName}**` : '';
    await channel.send({
