...
function bridgePingHandler(req, res) {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V19_TWISTER_2WAY_FIXED', now: Date.now() });
}
...
app.get('/', (_req, res) => {
  res.status(200).send('Santiago RP Verify Bot FAST V19 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V19_TWISTER_2WAY_FIXED',
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V18 activo en puerto ${CONFIG.port}`);
});
