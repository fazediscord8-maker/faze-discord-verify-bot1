...
const CONFIG = {
  ...
  // V12: timeout seguro configurable. Default 120s; máximo 180s.
...
function bridgePingHandler(req, res) {
  ...
  res.json({ ok: true, mode: 'FAST_QUEUE_V12_GET_BRIDGE_STABLE', now: Date.now() });
}
...
app.get('/', (_req, res) => {
  res.status(200).send('Santiago RP Verify Bot FAST V12 ONLINE');
});

app.get('/health', (_req, res) => {
  res.json({
    ok: true,
    mode: 'FAST_QUEUE_V12_GET_BRIDGE_STABLE',
...
async function handleInteraction(interaction) {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'faze_verify_modal') {
    ...
    // V12: no mensaje intermedio. Solo resultado final o corte seguro configurable.
    ...
    setTimeout(async () => {
      ...
      if (job && (job.status === 'pending' || job.status === 'processing')) {
        ...
        await safeEditReply(interaction, `❌ La verificación no recibió respuesta del servidor MTA después de 30 segundos.${bridgeMsg}\nNo vuelvas a intentarlo varias veces: avisa a staff para revisar el bridge.`);
      }
    }, CONFIG.jobTimeoutMs);
  }
}
...
app.listen(CONFIG.port, () => {
  console.log(`[OK] HTTP bridge FAST V12 activo en puerto ${CONFIG.port}`);
});
